                ...
                "Measure of a school’s ability to prepare its students for the next level of education, or the workforce."][i] %></p>
                  <% _.each(school[i + 1].scores, function(scoreYear) {
                    if (scoreYear.score === null) { %>
                      <div class="muted-label">
                          <p class="hed"><%= scoreYear.year %>&ensp;<i class="fa fa-minus-circle"></i></p>
                          <p class="txt">Not rated</p>
                      </div>
                    <% } else if (scoreYear.score < scoreYear.target) { %>
                      <div class="muted-label text-danger">
                          <p class="hed"><%= scoreYear.year %>&ensp;<i class="fa fa-times-circle"></i></p>
                          <p class="txt">Did not meet standard</p>
                          <p class="txt bold">Score: <%= scoreYear.score %></p>
                          <p class="txt">Target: <%= scoreYear.target %></p>
                      </div>
                    <% } else if (scoreYear.score >= scoreYear.target) { %>
                      <div class="muted-label text-success">
                          <p class="hed"><%= scoreYear.year %>&ensp;<i class="fa fa-check-circle"></i></p>
                          <p class="txt">Met standard</p>
                          <p class="txt bold">Score: <%= scoreYear.score %></p>
                          <p class="txt">Target: <%= scoreYear.target %></p>
                      </div>
